    ...
    func search(query: String, completion: @escaping (JCServerResponse<[JCSearchResult]>) -> Void) {
        ...
        networkManager.request(url: url, type: .get) { (response) in
            ...
            case .success(let data):
                guard let dto = try? JSONDecoder().decode(JCSearchResultDTO.self, from: data) else {
                    completion(JCServerResponse.failed(JCServerError()));
                    
                    guard let error = try? JSONDecoder().decode(JCServerError.self, from: data) else {
                        completion(JCServerResponse.failed(JCServerError())); return
                    }
                    completion(JCServerResponse.failed(error));
                    return
                }
                completion(JCServerResponse.success(dto.Search))
